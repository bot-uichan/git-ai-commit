...
```bash
curl -fL -o git-ai-commit \
  https://github.com/bot-uichan/git-ai-commit/releases/latest/download/git-ai-commit-linux-x64
mkdir -p ~/.local/bin && install -m 755 git-ai-commit ~/.local/bin/git-ai-commit
```
...
```bash
curl -fL -o git-ai-commit \
  https://github.com/bot-uichan/git-ai-commit/releases/latest/download/git-ai-commit-linux-arm64
mkdir -p ~/.local/bin && install -m 755 git-ai-commit ~/.local/bin/git-ai-commit
```
...
```bash
curl -fL -o git-ai-commit \
  https://github.com/bot-uichan/git-ai-commit/releases/latest/download/git-ai-commit-darwin-arm64
mkdir -p ~/.local/bin && install -m 755 git-ai-commit ~/.local/bin/git-ai-commit
```
...
```bash
curl -fL -o git-ai-commit \
  https://github.com/bot-uichan/git-ai-commit/releases/latest/download/git-ai-commit-darwin-x64
mkdir -p ~/.local/bin && install -m 755 git-ai-commit ~/.local/bin/git-ai-commit
```
...
```powershell
$dest = "$HOME\\bin\\git-ai-commit.exe"
New-Item -ItemType Directory -Force -Path (Split-Path $dest) | Out-Null
Invoke-WebRequest -Uri "https://github.com/bot-uichan/git-ai-commit/releases/latest/download/git-ai-commit-windows-x64.exe" -OutFile $dest
```
